...
function HeartsOverlay({glance}) {
  const [totalLikes, likesLoading] = [0, false];

  return (
    <div className="bg-black bg-opacity-50 absolute inset-0 w-full h-full hidden md:group-hover:flex">
      <div className="m-auto text-white font-medium text-lg flex items-center">
        <Heart className="w-5 h-5" />
        <span className="ml-1">
          {likesLoading ? '...' : Number(totalLikes || 0).toLocaleString()}
        </span>
      </div>
    </div>
  );
